...
test('has all divisions', async ({ page }) => {
  ...
  expect(events[0].summary).toBe(
    'Squash League - Division 3 - vs NEXUS 1 (HOME)'
  );
  expect(events[0].location).toBe('Cornwall Street Squash Centre');
  expect(events[0].start.toString()).toBe(
    'Tue Oct 08 2024 19:00:00 GMT+0800 (Hong Kong Standard Time)'
  );

  expect(events[17].summary).toBe(
    'Squash League - Division 3 - vs Hong Kong Football Club 3B (HOME)'
  );
  expect(events[17].location).toBe('Cornwall Street Squash Centre');
  expect(events[17].start.toString()).toBe(
    'Tue Mar 11 2025 19:00:00 GMT+0800 (Hong Kong Standard Time)'
  );
});
